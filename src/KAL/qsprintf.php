 ...
 * @group storage
 */
function qsprintf(KAL_QueryEscape $conn, $pattern/*, ... */) {
  $args = func_get_args();
  array_shift($args);
 ...
 * @group storage
 */
function vqsprintf(KAL_QueryEscape $conn, $pattern, array $argv) {
...
function xsprintf_query($userdata, &$pattern, &$pos, &$value, &$length) {
  ...
  if (!($conn instanceof KAL_QueryEscape)) {
...
function _qsprintf_check_type($value, $type, $query) {
  switch ($type) {
    case 'Ld': case 'Ls': case 'LC': case 'LA': case 'LO':
      if (!is_array($value)) {
        throw new KAL_QueryParameterException(
          $query,
